...
class tiktok:
    ...
    def update_title(self):
        ctypes.windll.kernel32.SetConsoleTitleW(f"TikTok Share Botter | Sent Shares: {self.shared} | Errors: {self.errors} | https://github.com/befalls")
    ...
    def main(self):
        os.system(clear)
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW(f"TikTok Share Botter | https://github.com/befalls")
        print(Fore.LIGHTRED_EX + ascii_text)
        tiktok_url = str(input(f"       {Fore.WHITE}[{Fore.RED}Console{Fore.WHITE}] TikTok URL: "))
        threads = int(input(f"       {Fore.WHITE}[{Fore.RED}Console{Fore.WHITE}] Threads: "))
        video_id = self.parse_url(tiktok_url)
        print(f"\n       {Fore.WHITE}[{Fore.RED}Console{Fore.WHITE}] Sending shares...")
        print()
        def thread_starter():
